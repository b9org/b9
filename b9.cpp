...
void bc_push_from_arg(ExecutionContext *context, StackElement *args,
                      Parameter offset) {
  push(context, args[offset]);
}

void bc_pop_into_arg(ExecutionContext *context, StackElement *args,
                     Parameter offset) {
  args[offset] = pop(context);
}

void bc_drop(ExecutionContext *context) { pop(context); }
...
Parameter bc_jmp_eq(ExecutionContext *context, Parameter delta) {
...
Parameter bc_jmp_neq(ExecutionContext *context, Parameter delta) {
  StackElement right = pop(context);
  StackElement left = pop(context);
  if (left != right) {
    return delta;
...
Parameter bc_jmp_gt(ExecutionContext *context, Parameter delta) {
  StackElement right = pop(context);
  StackElement left = pop(context);
  if (left > right) {
    return delta;
...
Parameter bc_jmp_ge(ExecutionContext *context, Parameter delta) {
  StackElement right = pop(context);
  StackElement left = pop(context);
  if (left >= right) {
    return delta;
...
Parameter bc_jmp_lt(ExecutionContext *context, Parameter delta) {
  StackElement right = pop(context);
  StackElement left = pop(context);
  if (left < right) {
    return delta;
...
Parameter bc_jmp_le(ExecutionContext *context, Parameter delta) {
  StackElement right = pop(context);
  StackElement left = pop(context);
  if (left <= right) {
    return delta;
...
StackElement interpret(ExecutionContext *context, Instruction *program) {
  ...
  while (*instructionPointer != NO_MORE_BYTECODES) {
    ...
    switch (getByteCodeFromInstruction(*instructionPointer)) {
      case PUSH_CONSTANT:
        bc_push_constant(context,
                         getParameterFromInstruction(*instructionPointer));
        break;
      case PUSH_STRING:
        bc_push_string(context,
                       getParameterFromInstruction(*instructionPointer));
        break;
      case DROP:
        bc_drop(context);
      ...
      case JMP:
        ...
        break;
      case JMP_EQ:
        instructionPointer += bc_jmp_eq(
            context, getParameterFromInstruction(*instructionPointer));
        break;
      case JMP_NEQ:
        instructionPointer += bc_jmp_neq(
            context, getParameterFromInstruction(*instructionPointer));
        break;
      case JMP_GT:
        instructionPointer += bc_jmp_gt(
            context, getParameterFromInstruction(*instructionPointer));
        break;
      case JMP_GE:
        instructionPointer += bc_jmp_ge(
            context, getParameterFromInstruction(*instructionPointer));
        break;
      case JMP_LT:
        instructionPointer += bc_jmp_lt(
            context, getParameterFromInstruction(*instructionPointer));
        break;
      case JMP_LE:
        instructionPointer += bc_jmp_le(
            context, getParameterFromInstruction(*instructionPointer));
        break;
      case CALL:
        ...
        break;
      case PUSH_FROM_VAR:
        bc_push_from_arg(context, args,
                         getParameterFromInstruction(*instructionPointer));
        break;
      case POP_INTO_VAR:
        bc_pop_into_arg(context, args,
                        getParameterFromInstruction(*instructionPointer));
        break;
      case RETURN: {
...
uint64_t getJitAddress(ExecutionContext *context, int functionIndex) {
...
bool loadLibrary(ExecutionContext *context, const char *libraryName) {
  if (context->library != nullptr) {
    printf("Error loading %s: context already has a library loaded",
           libraryName);
    return false;
  }
...
StackElement runFunction(ExecutionContext *context, Instruction *function) {
  if (context->stackPointer != context->stack) {
    printf("runProgram: Warning Stack not Empty (%ld elements)\n",
           context->stackPointer - context->stack);
    printf(
        "Possibly a prior run left items on stack, resetting the stack to a "
        "clean point.\n");
  ...
  if (context->stackPointer != context->stack) {
    printf(
        "runProgram: Warning Stack not Empty after running program (%ld "
        "elements)\n",
        context->stackPointer - context->stack);
    resetContext(context);
  }
...
extern "C" void b9_prim_hash_table_put(ExecutionContext *context) {
  ...
  push(context, (StackElement)hashTable_put(context, (pHeap)ht, (hashTableKey)k,
                                            (hashTableKey)v));
}
