...
void encrypt(const Key& key, const Tweak& tweak, const Span<const char>& plaintext) {

}


void skein256_64(const Key256& key, const Tweak& tweak, const std::uint64_t[4]& plain) {
...
private:
	...
	static const std::size_t ROTATE_BITS[] = {
		14, 16,
		52, 57,
		23, 40,
		5, 37,
	...
	static int getRotateBits(int d, int j) {
		return ROTATE_BITS[d*2 + j];
	}
